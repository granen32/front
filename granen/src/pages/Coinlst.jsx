...
const Coinlst = () => {
  ...
  const [data, setData] = useState([]);
    ...
    const fetchData = async () => {
      const res = await axios.get("https://api.coinpaprika.com/v1/tickers?limit=10");
      setData(res.data);
      setLoading(false);
  ...
  return (
    <>
      <h1>The Coins! {data.length}</h1>
      {loading ? <strong>Loading...</strong> : 
      (
      <>
        <select onChange={coinonChange}>
          {data.map((coin) => <option key={coin.quotes.USD.price} value={coin.rank}>{coin.name}(${coin.symbol}){coin.quotes.USD.price}</option>)}
        </select>
        <h2>coin convert</h2>
